...
const petSkinStyle = (skin?: string) => skin === 'starlight'
  ? 'ring-4 ring-violet-300/70 shadow-[0_0_28px_rgba(139,92,246,0.75)]'
  : skin === 'summer'
...
export default function ChatWindow({ peerId, peer, chatType, groupName, groupAvatar, onBack, initialOrbit = false, focusMessageId }: Props) {
  ...
  const petPendingForMe = pet?.status === 'pending' && pet.requestedBy !== user?.id;
    ...
    <div className="app-no-callout flex h-full flex-col relative overflow-hidden"
      ...
      {!isGroup && pet?.status === 'active' && (
        ...
        >
          <button
            ...
            className={`relative flex h-16 w-16 items-center justify-center overflow-hidden rounded-[24px] border-2 border-white bg-gradient-to-br ${petStage(pet.level).color} text-xl font-bold text-white shadow-xl ${petStage(pet.level).glow} ${petSkinStyle(pet.skin)}`}
            title="戳一戳宠物"
          >
          ...
          <div className="px-3 py-3 space-y-2 relative z-[1] min-h-full">
            {messages.map((msg) => {
              ...
              if (msg.type === 'pet-adopt') {
                ...
                const pendingForMe = event === 'requested' && msg.senderId !== user?.id && petPendingForMe;
      ...
      <Modal open={petOpen} onClose={() => setPetOpen(false)} title={'\u5171\u540c\u5ba0\u7269'}>
        {!pet ? (
          ...
          <div className="space-y-3">
            <p>{petPendingForMe ? '\u5bf9\u65b9\u9080\u8bf7\u4f60\u4e00\u8d77\u9886\u517b\u5ba0\u7269\u3002\u8bf7\u5728\u804a\u5929\u4e2d\u7684\u9080\u8bf7\u5361\u7247\u4e0a\u5904\u7406\u3002' : '\u9886\u517b\u9080\u8bf7\u5df2\u53d1\u9001\uff0c\u7b49\u5f85\u5bf9\u65b9\u786e\u8ba4\u3002'}</p>
          </div>
        ) : pet.status === 'rejected' ? (
          <div className="space-y-3">
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-xl bg-amber-50 p-3 dark:bg-amber-950/20">
              <div className="flex items-center justify-between text-xs text-amber-700 dark:text-amber-300">
                <span>{`\u6210\u957f\u7ecf\u9a8c ${pet.experience % 20}/20`}</span>
                <span>{`\u8ddd\u79bb\u4e0b\u4e00\u7ea7 ${20 - (pet.experience % 20)} \u7ecf\u9a8c`}</span>
              </div>
              <div className="mt-2 h-2 overflow-hidden rounded-full bg-amber-100 dark:bg-amber-900/30">
                <div className="h-full rounded-full bg-amber-500 transition-all" style={{ width: `${(pet.experience % 20) * 5}%` }} />
              </div>
