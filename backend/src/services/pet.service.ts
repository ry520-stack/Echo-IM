...
function nextLevel(experience: number) {
  return Math.max(1, Math.floor(experience / PET_EXP_PER_LEVEL) + 1);
}
...
export async function petAction(userId: string, peerId: string, action?: string) {
  ...
  if (!existing || existing.status !== 'active') throw new Error('Adopt a pet together first');
  const now = new Date();
  const grow = async (experienceGain: number, data = {}) => {
    const experience = existing.experience + experienceGain;
    await prisma.petBond.update({
      where: { id: existing.id },
      data: { ...data, experience, level: nextLevel(experience) },
    });
  ...
  if (action === 'buy-snack-box') {
    if (existing.coins < 12) throw new Error('金币不足');
    await grow(2, { coins: { decrement: 12 }, biscuits: { increment: 3 } });
    notifyPetUpdated(userId, peerId, 'bought-snack-box');
    return getPet(userId, peerId);
  }
  if (action === 'buy-repair-card') {
    if (existing.coins < 30) throw new Error('金币不足');
    await prisma.petBond.update({ where: { id: existing.id }, data: { coins: { decrement: 30 }, repairCards: { increment: 1 } } });
    notifyPetUpdated(userId, peerId, 'bought-repair-card');
    return getPet(userId, peerId);
  }
  if (action === 'buy-study-kit') {
    if (existing.coins < 16) throw new Error('金币不足');
    await grow(10, { coins: { decrement: 16 } });
    notifyPetUpdated(userId, peerId, 'bought-study-kit');
    return getPet(userId, peerId);
  }
  if (action === 'buy-date-pack') {
    if (existing.coins < 18) throw new Error('金币不足');
    await grow(4, { coins: { decrement: 18 }, intimacy: { increment: 4 } });
    notifyPetUpdated(userId, peerId, 'bought-date-pack');
    return getPet(userId, peerId);
  }
  if (action === 'feed') {
  ...
  if (action === 'cuddle') {
    await grow(2, { intimacy: { increment: 1 } });
    notifyPetUpdated(userId, peerId, 'cuddled');
    return getPet(userId, peerId);
  }
  const startActivity = async (activity: string, minutes: number, experienceGain: number, data = {}) => {
    await grow(experienceGain, { ...data, activity, activityUntil: new Date(now.getTime() + minutes * 60 * 1000) });
    notifyPetUpdated(userId, peerId, activity);
    return getPet(userId, peerId);
  };
  if (action === 'play') return startActivity('play', 10, 5, { intimacy: { increment: 2 } });
  if (action === 'walk') return startActivity('walk', 15, 5, { coins: { increment: 3 } });
  if (action === 'clean') return startActivity('clean', 8, 3, { intimacy: { increment: 1 } });
  if (action === 'bath') {
    if (existing.coins < 3) throw new Error('金币不足');
    return startActivity('bath', 10, 4, { coins: { decrement: 3 }, intimacy: { increment: 2 } });
  }
  if (action === 'train') {
    if (existing.coins < 6) throw new Error('金币不足');
    return startActivity('train', 20, 12, { coins: { decrement: 6 } });
  }
  if (action === 'explore') {
    if (existing.coins < 8) throw new Error('金币不足');
    return startActivity('explore', 25, 10, { coins: { decrement: 8 }, biscuits: { increment: 1 } });
  }
  if (action === 'dance') return startActivity('dance', 8, 4, { intimacy: { increment: 2 } });
  if (action === 'guard') return startActivity('guard', 15, 2, { coins: { increment: 2 } });
  if (action === 'study') {
    const experience = existing.experience + 4;
    await prisma.petBond.update({
      where: { id: existing.id },
      data: { activity: 'study', activityUntil: new Date(now.getTime() + 30 * 60 * 1000), experience, level: nextLevel(experience) },
    });
